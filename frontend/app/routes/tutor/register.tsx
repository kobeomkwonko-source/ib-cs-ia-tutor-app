...
export default function TutorRegisterRoute() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/tutor/login"), 800)
    } catch (error) {
      setForm((prev) => ({
        ...prev,
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Register as teacher</h1>
        <p className="text-sm text-gray-700">
          Create a teacher account to post homework tasks.
        </p>
      </div>
      ...
      <Card className="border">
        ...
        <CardFooter className="text-sm">
          ...
          <Button asChild variant="link" className="px-2">
            <Link to="/tutor/login">Go to teacher login</Link>
          </Button>
        </CardFooter>
      </Card>
    </div>
